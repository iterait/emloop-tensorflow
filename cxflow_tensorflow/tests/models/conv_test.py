...
class SimpleAutoencoder(cxtf.BaseModel):
    """
    ...
    def test_sanity(self):
        ...
        with tf.Graph().as_default(), tf.Session():
            ...
            with self.assertRaises(AssertionError):
                cxtf.models.cnn_encoder(x3, simple_encoder, use_bn=False)  # 3-dim input is not supported
            with self.assertRaises(AssertionError):
                cxtf.models.cnn_encoder(x6, simple_encoder, use_bn=False)  # 6-dim input is not supported
            with self.assertRaises(AssertionError):
                cxtf.models.cnn_encoder(x4, simple_encoder, use_bn=True)  # missing is_training
            with self.assertRaises(AssertionError):  # both ln and bn at once
                cxtf.models.cnn_encoder(x4, simple_encoder, is_training, use_bn=True, use_ln=True)

            with self.assertRaises(ValueError):
                cxtf.models.cnn_encoder(x4, ['12c3', '12c3', 'dunno'], use_bn=False)
        ...
        with tf.Graph().as_default(), tf.Session() as ses:
            ...
            for x in [x4, x5]:
                with tf.variable_scope('dim'+str(len(x.get_shape().as_list()))):
                    out = cxtf.models.cnn_encoder(x, ['12c3', '16inc', '12ress2', 'mp2'], is_training)
                    with tf.variable_scope('same_dim'):
                        same_dim_out = cxtf.models.cnn_encoder(x, ['12c3', '16inc', '12res', '3c3'], is_training)
...
class CNNAutoEncoderTest(TestCase):
    ...
    def test_sanity(self):
        ...
        with tf.Graph().as_default(), tf.Session():
            ...
            x5 = tf.ones((10, 15, 100, 100, 3))
            with self.assertRaises(AssertionError):
                cxtf.models.cnn_autoencoder(x3, simple_encoder)  # 3-dim input is not supported
            with self.assertRaises(AssertionError):
                cxtf.models.cnn_autoencoder(x5, simple_encoder)  # 5-dim input is not supported
            with self.assertRaises(AssertionError):
                cxtf.models.cnn_autoencoder(x4, ['mp2', '32c3'])  # first operation is pooling
            with tf.variable_scope('stride1'):
                with self.assertRaises(ValueError):
                    cxtf.models.cnn_autoencoder(x4, ['32c3', '32c3s2'])  # strided block does not have inversion
            with tf.variable_scope('stride2'):
                with self.assertRaises(ValueError):
                    cxtf.models.cnn_autoencoder(x4, ['32c3', '32ress2'])
            with tf.variable_scope('stride3'):
                with self.assertRaises(ValueError):
                    cxtf.models.cnn_autoencoder(x4, ['32c3', 'ap3s2'])
